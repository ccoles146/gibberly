...
class PubSubPublisher:
    def __init__(self, connection_string: str, hub: str = "sermon"):
        ...
        # HTTP-level timeouts + no retries prevent zombie threads when the PubSub REST
        # endpoint is slow or returns 429. Without retry_total=0, the default policy
        # retries 3× with backoff (4s × 3 + backoff ≈ 15s), filling the executor thread
        # pool and cascading timeouts across all subsequent chunks.
        ...
        self._client = WebPubSubServiceClient.from_connection_string(
            normalized_cs, hub=hub, transport=transport, retry_policy=RetryPolicy(retry_total=0)
        )
